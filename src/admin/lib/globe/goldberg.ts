/**
 * Goldberg polyhedron generator ported from the world editor.
 * A Goldberg(T) mesh has 10T^2 + 2 hexagonal/pentagonal faces.
 *
 * Read-only — node IDs in the Genesara world correspond to face indices.
 */
...
export function generateGoldberg(T: number): GoldbergMesh {
  ...
  const faces: GoldbergFace[] = geo.vertices.map((v, vi) => {
    ...
    for (let k = 0; k < sortedTris.length; k++) {
      ...
      neighbors.push(sharedInA.length >= 1 ? sharedInA[0] : -1);
    }
